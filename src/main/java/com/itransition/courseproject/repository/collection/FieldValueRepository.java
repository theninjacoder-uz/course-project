...
public interface FieldValueRepository extends JpaRepository<FieldValue, Long> {
    ...
    @Transactional
    @Modifying
    @Query("delete from FieldValue f where f.item.collection.id = ?1")
    int deleteAllByItem_CollectionId(Long item_collection_id);

    @Transactional
    @Modifying
    @Query("delete from FieldValue f where f.field in ?1")
    int deleteAllByFieldIn(Collection<Field> field);
}
